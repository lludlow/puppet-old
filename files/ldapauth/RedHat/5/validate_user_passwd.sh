...
LOGFILE="/var/log/passwd_merge`date +%y%m%d%H%M%S`.log"
...
if [ -z "${MTAB}" ]; then
   echo " Problem mounting linuxdist, password merging stopped" >> $LOGFILE
...
if [ ! -e "$MOUNTPOINT/user_maint/$HOSTNAME" ]; then
   echo "user password file $HOSTNAME missing on $MOUNTPOINT/user_maint, please create it before running the script" >> $LOGFILE
...
for ID in $IDS; do
        if [ $ID -le "$LIMIT" ];  then
           echo " Users have UID less than 1001, password merging stopped" >> $LOGFILE
...
for NAME in $NAMES; do
       ...
       if [ -n "${SHAD}" ]; then
           echo " Shadow entry exists for the user, user $NAME deleted from shadow file" >> $LOGFILE
...
for NAME in $NAMES; do
       ...
       if [ -n "${MSYS}" ]; then
           echo " User exist in the master system account file, password merging stopped" >> $LOGFILE
...
for NAME in $NAMES; do
       ...
       if [ "${MUSR}" ]; then
           ...
           echo " User $NAME exists in the master user account file" >> $LOGFILE
      else
           echo " User $NAME does not exist in the master user account file" >> $LOGFILE
